<?php
	...
	if(isset($_POST['title']))
	{
		...
		if($title!='')
		{
			...
			$insert = 'INSERT INTO questions VALUES("", "'.$title.'", "'.$desc.'", "'.$_SESSION['id'].'", 0, 0, "'.$time.'")';
			...
			if($tagList != '')
			{
				...
				foreach($tags as $tag)
				{
					...
					if($tag != '')
					{
						$query = 'SELECT * FROM storedTags WHERE name="'.$tag.'"';
					
						$query_run = mysql_query($query);
						
						if(mysql_num_rows($query_run)==0)
						{
							$insert = 'INSERT INTO storedTags VALUES("'.$tag.'")';
							
							mysql_query($insert);
						}
						
						$insert = 'INSERT INTO tags VALUES("'.$tag.'", '.$questionId.')';
			...
			header('Location:shareQuestion.php');

			die();
		}
		else
		{
			header('Location:questionEmpty.php'); //user hasn't filled all fields
	...
	{
		header('Location:askQuestion.php'); //user hasn't come here through the form on askQuestion
